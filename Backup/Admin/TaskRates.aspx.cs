...
{
    public partial class TaskRates : PageBase
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            rs = new DataReader("SELECT tr.ID, tr.FaciltityTaskID, tr.EmployeeID, ft.PayType, tr.EffectiveDate, tr.ExpirationDate, tr.LastModifiedBy, tr.LastModifiedOn, ft.FacilityId, ft.TaskDescription, e.LastName, e.FirstName, e.EmployeeNumber  FROM dbo.TaskRates tr LEFT OUTER JOIN  dbo.Employee e ON tr.EmployeeID = e.Id LEFT OUTER JOIN   dbo.FacilityTask ft ON tr.FaciltityTaskID = ft.Id  WHERE (e.Active = 1) AND (ft.Active = 1)  AND tr.FaciltityTaskID = " + Replace(rs__MMPARAM, "'", "''") + "  ORDER BY LastName, FirstName, EmployeeNumber ASC, ExpirationDate DESC");
            rs.Open();
            rs_numRows = 0;
