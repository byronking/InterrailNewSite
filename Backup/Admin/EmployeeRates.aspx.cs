...
{
    public partial class EmployeeRates : PageBase
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if(cStr(Request["MM_update"]) != "" && cStr(Request["MM_recordId"]) != ""){
              ...
              MM_editAction = "EmployeeRates.aspx";
              string MM_editRedirectUrl = "EmployeeRates.aspx?ID="+cStr(Request["MM_recordId"] + "&ShiftID=" + sDefShiftID);


              string MM_editQuery = "";
              // create the sql update statement;
              if(cStr(Request["Action"]) == "ADDNEWRATE"){
                    MM_editQuery =                  " Insert Into EmployeeRates ";
                    ...
                    MM_editQuery = MM_editQuery   + "   " + cStr(Session["FacilityID"])         + ", ";
                    ...
                    MM_editQuery = MM_editQuery   + "  '" + cStr(Session["UserName"])      + "', ";
                    MM_editQuery = MM_editQuery   + "  '" + cStr(Date())             + "') ";
              }else{
                MM_editQuery = "";
             }

              int numRec = cInt(Request["NRec"]);
              if(numRec > 0){
                for(int I=1; I <= numRec; I++){
                       if(Request["DELETE_"+cStr(I)] == "on"){
                         MM_editQuery = MM_editQuery + " Delete From EmployeeRates Where ID = " + cStr(Request["RecID_"+cStr(I)]) + " AND ShiftID = " + sDefShiftID;
                       }else{
                         MM_editQuery = MM_editQuery + " Update EmployeeRates SET " ;
            ...
            string strSQL = "";
            strSQL += " SELECT dbo.EmployeeRates.ID, dbo.EmployeeRates.FacilityID, dbo.EmployeeRates.TaskID, dbo.EmployeeRates.EmployeeID, ";
            strSQL +=  "dbo.EmployeeRates.UnitsPayRate, dbo.EmployeeRates.HoursPayRate, dbo.EmployeeRates.EffectiveDate,  ";
            strSQL +=  "dbo.EmployeeRates.LastModifiedBy, dbo.EmployeeRates.LastModifiedOn,  dbo.Tasks.TaskCode, ";
            strSQL +=  "dbo.Tasks.PayType  ";
            strSQL +=  "FROM dbo.EmployeeRates INNER JOIN ";
            strSQL +=  "                      dbo.Tasks ON   ";
            strSQL +=  " dbo.EmployeeRates.TaskID = dbo.Tasks.Id ";
            strSQL +=  "  WHERE (dbo.Tasks.Rebillable = 0 OR (dbo.Tasks.Rebillable = 1 AND TaskCode='RB')  ) ";
            strSQL +=  " AND dbo.EmployeeRates.EmployeeID = " + Replace(rs__MMColParam, "'", "''")  ;
            strSQL +=  "AND dbo.EmployeeRates.ShiftID = " + sDefShiftID ;
            strSQL +=  "AND dbo.EmployeeRates.FacilityID = " + Session["FacilityID"] ;
            strSQL +=  "ORDER BY TaskCode ASC, EffectiveDate DESC";
            ...
            rsTasks = new DataReader("SELECT Id=0, TaskDescription='Task', TaskCode=' Task? ', PayType='EITHER'  UNION  SELECT dbo.Tasks.Id, dbo.Tasks.TaskDescription, dbo.Tasks.TaskCode, dbo.Tasks.PayType  FROM dbo.Tasks , dbo.FacilityTasks  WHERE (dbo.Tasks.Rebillable = 0 OR (dbo.Tasks.Rebillable = 1 AND TaskCOde='RB')  ) AND dbo.Tasks.Id = dbo.FacilityTasks.TaskId  AND FacilityID = " + Replace(rsTasks__PFacID, "'", "''") + "  ORDER BY TaskCode");
            rsTasks.Open();
            ...
            strSQL = "";
            strSQL +=  "SELECT Id, ";
            strSQL +=  "Case when FacilityID <>  " + Session["FacilityID"] + " then 1 else 0 end as FromAssociatedFacility,  ";
            strSQL +=  "Case When (TempEmployee=0) Then EmployeeNumber Else TempNumber End As EmpNum, ";
            strSQL +=  "LastName, FirstName,  Case When (Salaried<>0) Then ' (Salaried) ' Else '' End As SalariedEmployee, ";
            strSQL +=  "Case When (TempEmployee=0) Then ' ' Else ' (Temp. Employee) ' End As EmpStatus  ";
            strSQL +=  "FROM dbo.Employee  WHERE ( FacilityID = " + Replace(rsEmployee__PFACID, "'", "''") ;
            strSQL +=  " OR  ( FacilityID IN ( Select AssociatedFacilityID from AssociatedFacility where FacilityID = " + rsEmployee__PFACID + ")) )  ";
            strSQL +=  " ORDER BY FromAssociatedFacility, LastName, FirstName";

            rsEmployee = new DataReader(strSQL);
            rsEmployee.Open();
