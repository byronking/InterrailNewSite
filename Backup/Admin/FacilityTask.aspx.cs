...
{
    public partial class FacilityTask : PageBase
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            rs__PFacilityID = "6";
            if(Session["FacilityID"] != ""){rs__PFacilityID = System.Convert.ToString(Session["FacilityID"]) ;}
            ...
            rs = new DataReader("SELECT DISTINCT  dbo.FacilityTask.Id, dbo.FacilityTask.TaskCode, dbo.FacilityTask.TaskDescription, f.Name, dbo.FacilityTask.FacilityId, dbo.FacilityTask.Active  FROM dbo.FacilityTask INNER JOIN  dbo.Facility f ON dbo.FacilityTask.FacilityId = f.Id  WHERE (f.Id =" + Replace(rs__PFacilityID, "'", "''") + " )  OR ('" + Replace(rs__PUserType, "'", "''") + "' = 'Admin') OR ('" + Replace(rs__PUserType, "'", "''") + "'='Super')  ORDER BY f.Name, TaskDescription ASC");
            rs.Open();
            rs_numRows = 0;
