<%@ Page Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="EmployeeRates.aspx.cs" Inherits="InterrailPPRS.Admin.EmployeeRates" %>
...
function goAdd()
{
  ...
  if (goValidate())
  {
    ...
    frm.action = "EmployeeRates.aspx?Action=ADDNEWRATE";
    frm.submit();
  }
}
...
function goSave()
{
  ...
  if (goValidate())
  {
    ...
    frm.action = "EmployeeRates.aspx?Action=UPDATE";

    frm.submit();
  }
}

function goEmployeeRates()
{
  ...
  for (i = 0; i < iend; i++)
  {
    if (frm.selEmployee.options[i].value == '<%=Request["ID"]%>')
        ...
        //    alert(frm.selEmployee.options[frm.selEmployee.options.selectedIndex].text);
        }
  }

  self.location = "EmployeeRates.aspx?ID="+empID+"&ShiftID="+shiftID;
...
<tr>
    ...
    <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%">
        ...
        <tr>
            <td align="default" width="15%" >Go to: <a href="EmployeeRebillRates.aspx?ID=<%=Request.QueryString["ID"]%>&Shift=<%=sDefShiftID%>">Rebill Rates</a></td>
            <td align="default" >&nbsp;</td>
        ...
        <tr>
            ...
            <select name="selEmployee" onChange='goEmployeeRates();'>
                <%
            while (! rsEmployee.EOF){
                rsEmployee.Read();
             %>
                <option value="<%=(rsEmployee.Item("Id")) %>" <% if ( cStr(rsEmployee.Item("Id")) == System.Convert.ToString(Request["ID"])) { Response.Write("SELECTED"); }else{ Response.Write(""); }%>><%=(rsEmployee.Item("LastName")) %>, <%=(rsEmployee.Item("FirstName")) %> (<%=(rsEmployee.Item("EmpNum")) %>) <% if (rsEmployee.Item("FromAssociatedFacility") == "1" ){ Response.Write("+"); } %> &nbsp; <%=(rsEmployee.Item("SalariedEmployee")) %> &nbsp;  <%=(rsEmployee.Item("EmpStatus")) %></option>
                <%
            }
            ...
            <td>
            Shift:
                    <select name="selShiftID" style="width:65px"   onChange='goEmployeeRates();'>
                    <%
            while (! rsShifts.EOF){
                rsShifts.Read();
            %>
        ...
        <tr class="reportOddLine">
            ...
            <select name="newTask"  onChange="showHideRate();">
            <%
            while (! rsTasks.EOF){
                rsTasks.Read();
            %>
            <option value="<%=(rsTasks.Item("Id")) %>"><%=Trim(rsTasks.Item("TaskCode")) %></option>
            <%
            }
        ...
        %>
        <tr class="<%=rowColor%>">
            <td align="default" width="15%"><%=rs.Item("TaskCode")%>
            ...
            <%}else{ %>
                <% if (InStr(0, "Super", System.Convert.ToString(Session["UserType"]), 1) >=1) { %>
                    <input value="<%=(rs.Item("UnitsPayRate")) %>" type="text" name="UR_<%=RecID%>"  maxlength="5" size="5">
                <% }else{ %>
                    <input value="<%=(rs.Item("UnitsPayRate")) %>" type="hidden" name="UR_<%=RecID%>" >
                    N/A
                <% } %>
            <%} %>
            </td>
            ...
            <%}else{ %>
                <% if (InStr(0, "Super", System.Convert.ToString(Session["UserType"]), 1) >=1) { %>
                        <input value="<%=(rs.Item("HoursPayRate")) %>" type="text" name="HR_<%=RecID%>"  maxlength="5" size="5">
                <% }else{ %>
                        <input value="<%=(rs.Item("HoursPayRate")) %>" type="hidden" name="HR_<%=RecID%>">
                        N/A
                <% } %>
            <%} %>
            </td>
            <td align="default" width="20%">
                <% if (InStr(0, "Super", System.Convert.ToString(Session["UserType"]), 1) >=1) { %>
                <input value="<%=(FormatDate(DateTime.Parse(rs.Item("EffectiveDate")))) %>" type="text" name="EFD_<%=RecID%>" size="10" maxlength="10">
                <% }else{ %>
                <%=(rs.Item("EffectiveDate")) %>
                <input value="<%=(FormatDate(DateTime.Parse(rs.Item("EffectiveDate")))) %>" type="hidden" name="EFD_<%=RecID%>" size="10" maxlength="10">
                <% } %>
            </td>
            <td align="default" width="15%">
                <% if (InStr(0, "Super", System.Convert.ToString(Session["UserType"]), 1) >=1) { %>
