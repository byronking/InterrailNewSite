...
{
    public partial class FacilityCustomerRebillSubTask : PageBase
    {

        public DataReader rsRebill;
        public DataReader MM_rs;
        public int rsRebill_total = 0;
        public int rsRebill_first = 0;
        public int rsRebill_last = 0;
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            string rsRebill__PFacilityID;
            rsRebill__PFacilityID = "2";
            if(Session["FacilityID"]   != ""){rsRebill__PFacilityID = System.Convert.ToString(Session["FacilityID"])  ;}

                GrantAccess("Super, Admin, User");


            rsRebill = new DataReader("SELECT r.Id, r.TaskID, r.Description, r.FacilityCustomerId, r.LastModifiedBy, r.LastModifiedOn,    dbo.Tasks.TaskCode, dbo.Tasks.TaskDescription  FROM dbo.Tasks INNER JOIN  dbo.FacilityCustomerRebillSubTasks r ON dbo.Tasks.Id = r.TaskID LEFT OUTER JOIN                        dbo.Facility f INNER JOIN                        dbo.FacilityCustomer fc ON f.Id = fc.FacilityId ON r.FacilityCustomerId = fc.Id  WHERE (f.Id = " + Replace(rsRebill__PFacilityID, "'", "''") + ")");
            rsRebill.Open();
            rsRebill_numRows = 0;

            Repeat1__numRows = 10;
            Repeat1__index = 0;
            rsRebill_numRows = rsRebill_numRows + Repeat1__numRows;


            //  *** Recordset Stats, Move To Record, && Go To Record: declare stats variables;

            // the record count;
            rsRebill_total = rsRebill.RecordCount;

            // the number of rows displayed on this page;
            if(rsRebill_numRows < 0){
                rsRebill_numRows = rsRebill_total;
            }else{
                if(rsRebill_numRows == 0){
                    rsRebill_numRows = 1;
                }
            ...
            rsRebill_first = 1;
            rsRebill_last  = rsRebill_first + rsRebill_numRows - 1;

            // if we have the correct record count, check the other stats;
            if(rsRebill_total != -1){
                if(rsRebill_first > rsRebill_total){rsRebill_first = rsRebill_total;}
                if(rsRebill_last > rsRebill_total){rsRebill_last = rsRebill_total;}
                if(rsRebill_numRows > rsRebill_total){rsRebill_numRows = rsRebill_total;}
            }
            ...
            if(rsRebill_total == -1){
                ...
                while (!rsRebill.EOF){
                    rsRebill.Read();
                    rsRebill_total = rsRebill_total + 1;
                ...
                // the number of rows displayed on this page;
                if(rsRebill_numRows < 0 || rsRebill_numRows > rsRebill_total){
                rsRebill_numRows = rsRebill_total;
                }
            ...
            MM_rs    = rsRebill;
            int MM_rsCount   = rsRebill_total;
            int MM_size      = rsRebill_numRows;
            string MM_uniqueCol = "";
            MM_paramName = "";
            MM_offset = 0;
            MM_atTotal = false;
            ...
            // the first && last displayed record;
            rsRebill_first = MM_offset + 1;
            rsRebill_last  = MM_offset + MM_size;
            if(MM_rsCount != -1){
                if(rsRebill_first > MM_rsCount){rsRebill_first = MM_rsCount;}
                if(rsRebill_last > MM_rsCount){rsRebill_last = MM_rsCount;}
            }
