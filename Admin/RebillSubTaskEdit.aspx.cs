...
{
    public partial class RebillSubTaskEdit : PageBase
    {
        ...
        if(cStr(Request["ID"]) == "0"){
          ...
          //strSQL = "SELECT * FROM dbo.RebillSubTasks WHERE Id =-1 ";
          ...
          rsRebill= new DataReader("SELECT Id, TaskID, Description,  FacilityCustomerId, HoursOrUnits, LastModifiedBy,  LastModifiedOn,  Active  FROM dbo.RebillSubTasks  WHERE Id = " + Replace(rsRebill__MM_ColParam, "'", "''") + "");
          rsRebill.Open();
          rsRebill.Read();
        ...
        if(Session["FacilityId"] != null && Session["FacilityId"]  != ""){rsCustomer__MM_FacilityId = System.Convert.ToString(Session["FacilityId"]);}
        ...
        rsTasks__PTaskID = "0";
        if (TheTaskID != null && TheTaskID != "") { rsTasks__PTaskID = TheTaskID; }
