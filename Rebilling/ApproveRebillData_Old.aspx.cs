...
{
    public partial class ApproveRebillData_Old : PageBase
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            rs = new DataReader("SELECT dbo.RebillDetail.Id, dbo.RebillDetail.WorkDate, dbo.RebillDetail.TotalHours, dbo.RebillDetail.TotalUnits, dbo.RebillDetail.RebillStatus, dbo.RebillSubTasks.Description,dbo.RebillSubTasks.HoursOrUnits,dbo.Tasks.TaskCode, dbo.FacilityCustomer.CustomerCode, dbo.FacilityCustomer.CustomerName,         iOrder = Case RebillStatus When 'OPEN' Then 1 When 'FACILITY' Then 2 When 'CORPORATE' Then 3 When 'SUPER' Then 4 Else 0 End  FROM         dbo.RebillDetail INNER JOIN                        dbo.RebillSubTasks ON dbo.RebillDetail.RebillSubTasksId = dbo.RebillSubTasks.Id INNER JOIN                        dbo.Tasks ON dbo.RebillSubTasks.TaskID = dbo.Tasks.Id INNER JOIN                        dbo.FacilityCustomer ON dbo.RebillSubTasks.FacilityCustomerId = dbo.FacilityCustomer.Id  WHERE RebillDetail.FacilityID=" + Replace(rs__PFacilityID, "'", "''") + "  ORDER BY iOrder ASC, WorkDate DESC,  dbo.RebillDetail.Id DESC" );
            rs.Open();
        ...
        {
            Response.Redirect("ApproveRebillDataEdit.aspx?Approval=CORPORATE&Which=OPEN&From=" + txtFromDate.Text + "&To=" + txtToDate.Text);
        }
    }
}