...
namespace InterrailPPRS.Production
{
    public partial class Detail : PageBase
    {
        ...
        public DataReader rs, rsTasks, rsID;
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            rs__PFacilityID = "2";
            if(Session["FacilityID"] != null && Session["FacilityID"]   != ""){rs__PFacilityID = System.Convert.ToString(Session["FacilityID"]);}

            string rstrSQL = "";
            rstrSQL = rstrSQL + "          SELECT Distinct WorkDate FROM FacilityProductionDetail                     ";
            rstrSQL = rstrSQL + "           WHERE 1 = 1                                                               ";
            rstrSQL = rstrSQL + "             AND FacilityProductionDetail.FacilityID = " + Session["FacilityID"];
            rstrSQL = rstrSQL + "          UNION ";
            rstrSQL = rstrSQL + "          SELECT Distinct WorkDate FROM NoProductionData                             ";
            rstrSQL = rstrSQL + "           WHERE 1 = 1                                                               ";
            rstrSQL = rstrSQL + "             AND NoProductionData.FacilityID = " + Session["FacilityID"];
            rstrSQL = rstrSQL + "        ORDER BY WorkDate Desc                                                       ";



            //rs.Source = "SELECT FacilityProductionDetail.WorkDate, SUM(FacilityProductionDetail.Units) AS TotalUnits, COUNT(*) AS NREC, MAX(FacilityProductionDetail.Id) AS ID,                         Tasks = //LO && UL// , ApprovalStatus FROM         FacilityProductionDetail INNER JOIN                        Tasks ON FacilityProductionDetail.TaskId = Tasks.Id  WHERE     ((Tasks.TaskCode = //LO// OR                        Tasks.TaskCode = //UL//)  && (FacilityID =  " + Replace(rs__PFacilityID, "//", "////") + "))  GROUP BY FacilityProductionDetail.WorkDate, ApprovalStatus  UNION        SELECT     FacilityProductionDetail.WorkDate, SUM(FacilityProductionDetail.Units) AS TotalUnits, COUNT(*) AS NREC, MAX(FacilityProductionDetail.Id) AS ID,                         Tasks =  //Other//, ApprovalStatus  FROM FacilityProductionDetail INNER JOIN                        Tasks ON FacilityProductionDetail.TaskId = Tasks.Id  WHERE ((Tasks.TaskCode != //LO// &&                        Tasks.TaskCode != //UL//)  && (FacilityID =  " + Replace(rs__PFacilityID, "//", "////") + "))  GROUP BY FacilityProductionDetail.WorkDate, ApprovalStatus  ORDER BY WorkDate Desc, Tasks, ID";
            rs = new DataReader( rstrSQL);
            ...
            // *** Go To Record and Move To Record: create strings for maintaining URL && Form parameters;
            ...
            // *** Move To Record: the strings for the first, last, next, and previous links;
