...
{
    public partial class PayrollDetailReport4 : PageBase
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            string wDateRange = " AND (WorkDate Between '" + sfromDateDetail + "' AND " + stoDateDetail + "') ";

            string sWhere = wFacilities + wDateRange;
            

            string strSQL = "";
            strSQL = strSQL + "          SELECT IsNull(Count(*), 0) as theCount from  EmployeeTaskWorked   ";
            strSQL = strSQL + "           WHERE PayrollStatus = 'OPEN'  ";
            ...
            if ( iOpen > 0) {
                 ...
                 strSQL = "";
                 strSQL = strSQL + "    SELECT     Facility.Name ";
                 strSQL = strSQL + "    FROM         EmployeeTaskWorked INNER JOIN ";
                 strSQL = strSQL + "                          Employee ON EmployeeTaskWorked.EmployeeId = Employee.Id INNER JOIN ";
                 strSQL = strSQL + "                          Facility ON Employee.FacilityID = Facility.Id ";
                 strSQL = strSQL + "           WHERE EmployeeTaskWorked.PayrollStatus = 'OPEN'  ";

                 if(sselFacilities != ""){
                   strSQL = strSQL + "  AND (Employee.FacilityID  NOT IN  (" + sselFacilities + ") ) ";
                 }else{
                   strSQL = strSQL + "  AND (Employee.FacilityID  <> " + cStr(Session["FacilityID"]) + " ) ";
                 }
            ...
            strSQL = "";
            strSQL = strSQL + " SELECT     EmployeeTaskWorked.Id, EmployeeTaskWorked.FacilityID, EmployeeTaskWorked.WorkDate, ISNULL(EmployeeTaskWorked.UPM, 0) AS UPM, ";
            strSQL = strSQL + "                       Tasks.TaskCode, Facility.Name, 0 AS TaskType, Employee.EmployeeNumber, Employee.LastName, Employee.FirstName, Employee.ID as EmployeeID, ";
            strSQL = strSQL + "                       EmployeeTaskWorked.ShiftID, Facility.OvertimeCalcBasis, ";
            strSQL = strSQL + "                       ";
            strSQL = strSQL + "                       IsNull((select TOP 1 EmployeeTaskWorkedPay.PayRate from EmployeeTaskWorkedPay where EmployeeTaskWorkedPay.EmployeeTaskWorkedID = EmployeeTaskWorked.id and EmployeeTaskWorkedPay.PayMultiplier = 1.0),0) ";
            strSQL = strSQL + "                       as RegPayRate, ";
            strSQL = strSQL + "                       IsNull((select TOP 1 EmployeeTaskWorkedPay.HoursPaid from EmployeeTaskWorkedPay where EmployeeTaskWorkedPay.EmployeeTaskWorkedID = EmployeeTaskWorked.id and EmployeeTaskWorkedPay.PayMultiplier = 1.0),0) ";
            strSQL = strSQL + "                       as RegHoursPaid, ";
            strSQL = strSQL + "                       IsNull((select TOP 1 EmployeeTaskWorkedPay.PayAmount from EmployeeTaskWorkedPay where EmployeeTaskWorkedPay.EmployeeTaskWorkedID = EmployeeTaskWorked.id and EmployeeTaskWorkedPay.PayMultiplier = 1.0),0) ";
            strSQL = strSQL + "                       as RegPayAmount, ";
            strSQL = strSQL + "                       IsNull((select Avg(EmployeeTaskWorkedPay.PayRate) from EmployeeTaskWorkedPay where EmployeeTaskWorkedPay.EmployeeTaskWorkedID = EmployeeTaskWorked.id and EmployeeTaskWorkedPay.PayMultiplier > 1.0),0) ";
            strSQL = strSQL + "                       as OTPayRate, ";
            strSQL = strSQL + "                       IsNull((select SUM(EmployeeTaskWorkedPay.HoursPaid) from EmployeeTaskWorkedPay where EmployeeTaskWorkedPay.EmployeeTaskWorkedID = EmployeeTaskWorked.id and EmployeeTaskWorkedPay.PayMultiplier > 1.0),0) ";
            strSQL = strSQL + "                       as OTHoursPaid, ";
            strSQL = strSQL + "                       IsNull((select SUM(EmployeeTaskWorkedPay.PayAmount) from EmployeeTaskWorkedPay where EmployeeTaskWorkedPay.EmployeeTaskWorkedID = EmployeeTaskWorked.id and EmployeeTaskWorkedPay.PayMultiplier > 1.0),0) ";
            strSQL = strSQL + "                       as OTPayAmount ";
            strSQL = strSQL + " FROM         EmployeeTaskWorked INNER JOIN ";
            strSQL = strSQL + "                       Employee ON EmployeeTaskWorked.EmployeeId = Employee.Id  INNER JOIN ";
            ...
            strSQL = strSQL + " WHERE     (EmployeeTaskWorked.TaskID <> 0) AND (EmployeeTaskWorked.OtherTaskID = 0) AND ";
            strSQL = strSQL + "                       (HireDate <= WorkDate AND Employee.HireDate IS NOT NULL)  AND ";
            strSQL = strSQL + "                       (EmployeeTaskWorked.PayrollStatus <> 'OPEN')  ";
            strSQL = strSQL + sWhere;

            strSQL = strSQL + "      UNION   ";

            strSQL = strSQL + " SELECT     EmployeeTaskWorked.Id, EmployeeTaskWorked.FacilityID, EmployeeTaskWorked.WorkDate, ISNULL(EmployeeTaskWorked.UPM, 0) AS UPM, ";
            strSQL = strSQL + "                       OtherTasks.TaskCode, Facility.Name, 1 AS TaskType, Employee.EmployeeNumber, Employee.LastName, Employee.FirstName,  Employee.ID as EmployeeID, ";
            strSQL = strSQL + "                       EmployeeTaskWorked.ShiftID, Facility.OvertimeCalcBasis, ";
            strSQL = strSQL + "                       ";
            strSQL = strSQL + "                       IsNull((select TOP 1 EmployeeTaskWorkedPay.PayRate from EmployeeTaskWorkedPay where EmployeeTaskWorkedPay.EmployeeTaskWorkedID = EmployeeTaskWorked.id and EmployeeTaskWorkedPay.PayMultiplier = 1.0),0) ";
            strSQL = strSQL + "                       as RegPayRate, ";
            ...
            strSQL = strSQL + " FROM         EmployeeTaskWorked INNER JOIN ";
            strSQL = strSQL + "                       Employee ON EmployeeTaskWorked.EmployeeId = Employee.Id  INNER JOIN ";
            strSQL = strSQL + "                       OtherTasks ON EmployeeTaskWorked.OtherTaskID = OtherTasks.Id INNER JOIN ";
            strSQL = strSQL + "                       Facility ON EmployeeTaskWorked.FacilityID = Facility.Id ";
            strSQL = strSQL + " WHERE      (EmployeeTaskWorked.TaskID = 0) AND (EmployeeTaskWorked.OtherTaskID <> 0) AND ";
            strSQL = strSQL + "                       (HireDate <= WorkDate AND Employee.HireDate IS NOT NULL) AND ";
            strSQL = strSQL + "                       (EmployeeTaskWorked.PayrollStatus <> 'OPEN')  ";
            ...
            strSQL = strSQL + " ORDER BY EmployeeTaskWorked.FacilityID, Employee.LastName, Employee.FirstName, EmployeeTaskWorked.WorkDate, Tasks.TaskCode ";
            strSQL = strSQL + " ";
            ...
            strSQL = "";
            strSQL = strSQL + " SELECT     EmployeeTaskWorked.Id, EmployeeTaskWorked.FacilityID, EmployeeTaskWorked.WorkDate, ISNULL(EmployeeTaskWorked.UPM, 0) AS UPM, ";
            strSQL = strSQL + "                       Tasks.TaskCode, Facility.Name, 0 AS TaskType, Employee.TempNumber, Employee.LastName, Employee.FirstName,  Employee.ID as EmployeeID, ";
            strSQL = strSQL + "                       EmployeeTaskWorked.ShiftID, Facility.OvertimeCalcBasis, SourceName, EmploymentSource.Id AS SourceID, ";
            strSQL = strSQL + "                       ";
            strSQL = strSQL + "                       IsNull((select TOP 1 EmployeeTaskWorkedPay.PayRate from EmployeeTaskWorkedPay where EmployeeTaskWorkedPay.EmployeeTaskWorkedID = EmployeeTaskWorked.id and EmployeeTaskWorkedPay.PayMultiplier = 1.0),0) ";
            strSQL = strSQL + "                       as RegPayRate, ";
            strSQL = strSQL + "                       IsNull((select TOP 1 EmployeeTaskWorkedPay.HoursPaid from EmployeeTaskWorkedPay where EmployeeTaskWorkedPay.EmployeeTaskWorkedID = EmployeeTaskWorked.id and EmployeeTaskWorkedPay.PayMultiplier = 1.0),0) ";
            ...
            strSQL = strSQL + "                       Tasks ON EmployeeTaskWorked.TaskID = Tasks.Id INNER JOIN ";
            strSQL = strSQL + "                       Facility ON EmployeeTaskWorked.FacilityID = Facility.Id ";
            strSQL = strSQL + "                 INNER JOIN EmploymentSource                             ";
            strSQL = strSQL + "                 ON EmploymentSourceId = EmploymentSource.Id             ";
            strSQL = strSQL + " WHERE    ";
            strSQL = strSQL + "                       (HireDate Is NULL OR HireDate > WorkDate) AND (EmployeeTaskWorked.TaskID <>  0) AND (EmployeeTaskWorked.OtherTaskID = 0) AND ";
            strSQL = strSQL + "                       (EmployeeTaskWorked.PayrollStatus <> 'OPEN')  ";
            strSQL = strSQL + sWhere;
            ...
            strSQL = strSQL + " SELECT     EmployeeTaskWorked.Id, EmployeeTaskWorked.FacilityID, EmployeeTaskWorked.WorkDate, ISNULL(EmployeeTaskWorked.UPM, 0) AS UPM, ";
            strSQL = strSQL + "                       OtherTasks.TaskCode, Facility.Name, 1 AS TaskType, Employee.TempNumber, Employee.LastName, Employee.FirstName,  Employee.ID as EmployeeID, ";
            strSQL = strSQL + "                       EmployeeTaskWorked.ShiftID, Facility.OvertimeCalcBasis, SourceName, EmploymentSource.Id AS SourceID, ";
            strSQL = strSQL + "                       ";
            strSQL = strSQL + "                       IsNull((select TOP 1 EmployeeTaskWorkedPay.PayRate from EmployeeTaskWorkedPay where EmployeeTaskWorkedPay.EmployeeTaskWorkedID = EmployeeTaskWorked.id and EmployeeTaskWorkedPay.PayMultiplier = 1.0),0) ";
            strSQL = strSQL + "                       as RegPayRate, ";
    ...
    string strSQL, results;
     ...
     strSQL = "";
     strSQL = strSQL + "    SELECT     Facility.Name, Facility.ID, Facility.AlphaCode ";
     strSQL = strSQL + "    FROM        Employee INNER JOIN ";
     strSQL = strSQL + "                     Facility ON Employee.FacilityID = Facility.Id ";
     strSQL = strSQL + "           WHERE Employee.ID =  " + EmployeeID;
    ...
    public void ShowTempDetailReport(){
      ...
      if(rsTemp.EOF){
        ...
        while(! rsTemp.EOF){
          //Set rsTempPay = rsTemp.Item("rstPay").Value;
            rsTemp.Read();
          ...
          if(! rsTemp.EOF){
            if( (cStr(sEmpNum) != rsTemp.Item("TempNumber")) ){
