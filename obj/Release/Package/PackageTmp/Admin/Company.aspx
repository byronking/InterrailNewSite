...
<asp:Content ID="mainContent" ContentPlaceHolderID="Main" runat="server">
     ...
     <form id="allCompaniesForm" runat="server">
        <h2>InterRail Users</h2>
        <div style="float:right;"><asp:HyperLink ID="linkAddNewCompany" runat="server" Text="Add New Company" NavigateUrl="~\Admin\CompanyEdit.aspx?Id=0" /></div><p />
        <div><asp:TextBox ID="txtSearch" runat="server" />&nbsp;<asp:Button ID="btnSearch" runat="server" Text="Search" OnClick="btnSearch_Click" /></div><p />
        <div>
            <asp:GridView runat="server" ID="grdAllCompanies" AllowPaging="true" PageSize="15"
                OnPageIndexChanging="grdAllCompanies_PageIndexChanging" AlternatingRowStyle-BackColor="#DEDFFF" 
                AutoGenerateColumns="false" GridLines="Horizontal" EmptyDataText="No results found!">
                <PagerSettings Position="Bottom" />
                <Columns>
                    <asp:HyperLinkField DataTextField="CompanyId" DataNavigateUrlFields="Id" 
                        DataNavigateUrlFormatString="~\Admin\CompanyEdit.aspx?Id={0}" 
                        HeaderStyle-Width="150" ItemStyle-HorizontalAlign="Left" />
                    <asp:HyperLinkField DataTextField="CompanyName" DataNavigateUrlFields="Id" 
                        DataNavigateUrlFormatString="~\Admin\CompanyEdit.aspx?Id={0}" 
                        HeaderStyle-Width="350" ItemStyle-HorizontalAlign="Left" />
                </Columns>
            </asp:GridView>
        </div><p />
